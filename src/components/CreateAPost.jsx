...
const CreateAPost = () => {
    ...
    const imageRefs = useRef([]); // Array to store refs for each image input
    ...
    // const [imageNotes, setImageNotes] = useState([]); // Optional array for individual image notes
            ...
            <Modal isOpen={isDetailsOpen} onClose={onDetailsClose} scrollBehavior="inside" size="lg">
                ...
                <ModalContent border="1px solid gray">
                    ...
                    <ModalBody pb={6}>
                        ...
                        {/*<Textarea*/}
                        {/*    mt={4}*/}
                        {/*    placeholder="Add caption..." // Removed note as a general post caption*/}
                        {/*    value={imageCaptions[index] || ''}*/}
                        {/*    onChange={(e) => setImageCaptions([...imageCaptions, e.target.value].slice(-selectedFiles.length))} // Update imageCaptions with user input*/}
                        {/*/>*/}
                    </ModalBody>
                    <ModalFooter>
    ...
    const handleCreatePost = async (selectedFiles, imageTitles, imageCaptions, note) => {
        ...
        try {
            const postDocRef = await addDoc(collection(firestore, "postss"), postData
                // ...postData, // Include other post properties from newPost
                // note, // Add the title property from the component state
                // imageUrls: [], // Array to store download URLs for all images
                // createdAt: Date.now(), // Add timestamp if needed
                // createdBy: authUser.uid, // Replace with your user ID retrieval logic (if not already in newPost)
            // }
            );
            ...
            // newPost.imageUrls = imageUrls; // Update local post object (optional)
